...
# We address this issue with vega-lite, utilizing the `selection`, `opacity`, and `tooltip` features .
-->
---
...
<script>
  const spec = {
    ...
    "transform": [
     ...
     // Interaction: define a single-value selection named `vendorSelect`.
     // This selection is bound to an HTML <select> input (see `bind` below),
     // listens for `mouseover` events, and tracks the `vendor` field.
     // The selection is referenced by encodings (e.g., `opacity`) to highlight
     // the chosen vendor while dimming others; the dropdown options are defined
     // in the selection's `bind` configuration.
    "selection": {
      "vendorSelect": {
        "type": "single",
        "on": "mouseover",
    ...
    "encoding": {
      ...
      "color": {
        "field": "vendor",
        "type": "nominal",
        "legend": {"orient": "right", "title": "Vendor"},
